...
folder2 = folder +'dinamica10_Sandro/res57/res57ah/'

dados1 = pd.read_csv(folder2+'dados_57a.txt', header = None, sep = '|')
dados1.columns = dados_names

dados2 = pd.read_csv(folder2+'dados_57b.txt', header = None, sep = '|')
dados2.columns = dados_names

dados3 = pd.read_csv(folder2+'dados_57c.txt', header = None, sep = '|')
dados3.columns = dados_names

dados4 = pd.read_csv(folder2+'dados_57d.txt', header = None, sep = '|')
dados4.columns = dados_names

dados5 = pd.read_csv(folder2+'dados_57e.txt', header = None, sep = '|')
dados5.columns = dados_names

dados6 = pd.read_csv(folder2+'dados_57f.txt', header = None, sep = '|')
dados6.columns = dados_names

dados7 = pd.read_csv(folder2+'dados_57g.txt', header = None, sep = '|')
dados7.columns = dados_names

dados8 = pd.read_csv(folder2+'dados_57h.txt', header = None, sep = '|')
...
dados11 = pd.read_csv(folder3+'dados_50m.txt', header = None, sep = '|')
dados11.columns = dados_names
N11 = dados11.amostras[0]

folder5 = folder +'dinamica11_random/res59ad/'

dados59d = pd.read_csv(folder5+'dados_59d.txt', header = None, sep = '|')
dados59d.columns = dados_names
N59d = dados59d.amostras[0]

dados_random = ( N11 * dados11.custo + N59d * dados59d.custo ) / ( N11+N59d )
...
plt.loglog(dados3.Nv,dados3.custo,'^g', ms = 4, label = r'$v_0 = 0.5 $')
plt.loglog(dados4.Nv,dados4.custo,'pm', ms = 4, label = r'$v_0 = 1.0 $')
plt.loglog(dados5.Nv,dados5.custo,'vc', ms = 4, label = r'$v_0 = 1.5 $')
#plt.loglog(dados6.Nv,dados6.custo,'-v', color = 'orange', ms = 4, label = r'$v_0 = 5.0 $')
#plt.loglog(dados7.Nv,dados7.custo,'-1c', ms = 4, label = r'$v_0 = 10.0 $')
plt.loglog(dados8.Nv,dados8.custo,'or', ms = 4, label = r'$v_0 = 100.0 $')
plt.loglog(dados11.Nv,dados_random,'Dk', lw = 2, label = r'RN',
...
plt.axvline(x=19.2,color='darkred', linestyle='--')
plt.legend(loc='upper left',fontsize = 10)
plt.xlabel(r'Number of agents $M$',fontsize = 16)
plt.ylabel(r'Computational cost $C$',fontsize = 16)
plt.title(r'Caso \textit{quenched}: relevo $K = 4$, $\delta = 2.5$.',fontsize=14)
plt.grid(True)
plt.xticks(size=14)
plt.yticks(size=14)
axes = plt.gca()
axes.set_xlim([2,2e3])
axes.set_ylim([6e-1,7])
#plt.text(3,1.5,r'$\delta = 2.5$',fontsize = 14)#, color = 'b', backgroundcolor = 'w')
plt.savefig('figura_13a.pdf',dpi = 300, bbox_inches='tight') 
plt.close()
