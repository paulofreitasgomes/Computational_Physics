...
dados1 = pd.read_csv(folder2+'dados_49a.txt', header = None, sep = '|')
dados1.columns = dados_names

dados2 = pd.read_csv(folder2+'dados_49b.txt', header = None, sep = '|')
dados2.columns = dados_names

dados3 = pd.read_csv(folder2+'dados_49c.txt', header = None, sep = '|')
dados3.columns = dados_names

dados4 = pd.read_csv(folder2+'dados_49d.txt', header = None, sep = '|')
dados4.columns = dados_names

vp = math.sqrt(300)

#plt.plot(dados1.v0,dados1.custo,'-sb', ms = 4, label = r'$\delta = 1.4$')
...
#plt.title(r'$\textbf{v} = v\textbf{x}$, $p = 0.5$, $M = 300$, $\delta=2$, $v_p=17.32$.',fontsize=16)
plt.grid(True)
for i2 in [1,2,3,4]:
...
plt.savefig('figura_12a.pdf',dpi = 300, bbox_inches='tight') 
plt.close()
