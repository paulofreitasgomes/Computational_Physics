...
INTEGER, PARAMETER :: Q = 110, Nv = 16384, samples = 100, Nmax = 300    !Nv tem de ser par 
INTEGER :: seed, estado(Nv,Q), Nneig(Nv), neighbors(Nv,Nmax), ind0, viz
REAL, PARAMETER ::  v = 1.0, p_walk = 0.5, pi = 3.14159265 !p_walk = w
REAL :: L, pos_x(Nv,Q+1), pos_y(Nv,Q+1), dist2, rho, d_int, delxy, c_rec
...
REAL :: delta, cmin = 0.0, cmax = 0.12, num_iter
REAL :: numerador, denominador, ln_i_av, somaA, somaB, somaC, somaD, slope, intercept, alpha
INTEGER :: tc = 7, Gamma = 22
!Gamma = numero de valores de c_rec

!======================================================================================================!
open (unit=3,file="parametros_N16384_del0p9.txt",action="write",status="replace")
open (unit=8,file="contagens_N16384_del0p9.csv",action="write",status="replace")
open (unit=10,file="informacoes_N16384_del0p9.txt",action="write",status="replace")
open (unit=12,file="coeficiente_N16384_del0p9.txt",action="write",status="replace")
!======================================================================================================!  
...
!ajuste linear
somaA = 0.0; somaB = 0.0; somaC = 0.0; somaD = 0.0; num_iter = 0
DO t2=tc,Q
    somaA = somaA + LOG(1.0*t2)*LOG(cont_inf(t2))
    somaB = somaB + LOG(1.0*t2)
    somaC = somaC + LOG(cont_inf(t2))
    somaD = somaD + LOG(1.0*t2)**2     
    num_iter = num_iter + 1
ENDDO

IF (num_iter*somaD - somaB**2 .EQ. 0) print*,'Denominador do slope igual a zero!' 
slope = (num_iter*somaA-somaB*somaC)/(num_iter*somaD - somaB**2) !coeficiente angular
intercept = (somaC*somaD - somaB*somaA)/(num_iter*somaD - somaB**2) !coeficiente linear
...
numerador = 0.0; denominador = 0.0; ln_i_av = 0.0; alpha = 0

DO t4=tc,Q
    ln_i_av = ln_i_av + LOG(cont_inf(t4))/num_iter
ENDDO

! print*,'LOG(cont_inf(Q))',LOG(cont_inf(Q))
! print*,'ln_i_av',ln_i_av

DO t4=tc,Q
    numerador = numerador + (intercept + slope*LOG(1.0*t4) - ln_i_av)**2
    denominador = denominador + (LOG(cont_inf(t4)) - ln_i_av)**2
ENDDO

IF (denominador .EQ. 0) print*,'Denominador do alpha igual a zero!' 
alpha = numerador/denominador !coeficiente de determinacao

! print*,'numerador: ', numerador
! print*,'denominador: ', denominador
! print*,'alpha: ',alpha

write(12,*) c_rec, alpha, slope, intercept

ENDDO !fim do do em c_rec, para calculo do alpha
